...
def check_name_contain_update(filename):
    """Kiểm tra xem tên file có chứa chữ 'update' hay không."""
    return "update" in filename.lower()

def convert_filename_contain_update(filename):
  """Chuyển đổi tên file từ 'UPDATE_PVCFCCo_BCSX_20240326' thành 'PVCFCCo_BCSX_20240326'."""
  return re.sub(r"^UPDATE_", "", filename, flags=re.IGNORECASE)

def function_formatter(file_path_raw_data, file_path_data_formated, name_file, sheet_name, len_catalog, names_columns_sheet):
    
    absolute_file_path = os.path.join(file_path_raw_data, name_file)

    try:
        raw_data = pd.read_excel(file_path_raw_data + '\\' + name_file, dtype=str, header=None, sheet_name=sheet_name)
        print(raw_data.iloc[:, :3])
        # print(file_path_raw_data + '\\' + name_file)
        # print(raw_data)
        # # search skip rows
        ...
        if check_name_contain_update(name_file):
            name_file_convert = convert_filename_contain_update(name_file)
            old_file_name = file_path_raw_data + '\\' + name_file
            new_file_name = file_path_raw_data + '\\' + "_UPDATED_" + name_file_convert
            finally_df.to_excel(file_path_data_formated +  '\\RESULT_' + name_file_convert, index=False)
...
def on_created(event):
    ...
    files_raw_data      = module_process_file.check_file(module_process_file.list_files_pvcfcco(file_path_raw_data),False)
