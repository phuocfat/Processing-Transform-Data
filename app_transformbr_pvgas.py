...
pvgas_json          = info_json["PVGAS"]
file_path           = pvgas_json["URI_PVGAS"]
names_columns       = pvgas_json["NAMES_COLUMNS"]
len_catalog        = pvgas_json["LEN_CATALOG"]
...
def function_formatter(file_path_raw_data, file_path_data_formated, name_file, len_catalog):
    # Read file is xlsx from parameter configuration
    raw_data = pd.read_excel(file_path_raw_data + '\\' + name_file, dtype=float, header=None)
    print(raw_data.iloc[:20, :3])
    # search skip rows
    ...
    raw_data.rename(columns={0: names_columns[0], 1: names_columns[1], 2: names_columns[2], 3: names_columns[3], 4: names_columns[4], 5: names_columns[5], 6: names_columns[6], 7: names_columns[7], 8: names_columns[8]}, inplace=True)
    
    # delete raw_data.rename(columns={0: names_columns[0], 1: names_columns[1], 2: names_columns[2], 3: names_columns[3], 4: names_columns[4], 5: names_columns[5], 6: names_columns[6], 7: names_columns[7], 10: names_columns[8]}, inplace=True)
    
    df = raw_data.iloc[index:, :].copy()
    print(df.iloc[:10, :])
    # add date to column with name 'NGÀY BÁO CÁO'
    date_convert = module_process_string.convert_date(name_file)
    if date_convert != False:
        ...
        # insert columns with names such as "DANH MỤC 1", "DANH MỤC 2", and "DANH MỤC 3"
        df = module_process_data.insert_many_columns(df, len_catalog)
        
        # process transforms catalog
        ...
        finally_df = df.iloc[:,:12].copy()
        # finally_df.to_excel(file_path_data_formated +  '\\RESULT_' + name_file, index=False,float_format='%f')



function_formatter('E:\OneDrive-BI_Sync\OneDrive\PVGas\Input', 'E:\OneDrive','PV GAS_BCSX_ 20240117.xlsx',len_catalog)
...
#def on_created(event):
    ...
    #     function_formatter(file_path_raw_data, file_path_data_formated,result_list[i], len_catalog)


#def watch_directory_app_pvgas(path):
    ...
    #     observer.stop()
    # observer.join()
#watch_directory_app_pvgas(file_path)