...
pvgas_json          = info_json["BSR"]
file_path           = pvgas_json["URI_BSR"]
...
def function_formatter(file_path_raw_data, file_path_data_formated, name_file, len_catalog, len_columns):
    ...
    raw_data.rename(columns={0: names_columns[0], 1: names_columns[1], 2: names_columns[2], 3: names_columns[3], 4: names_columns[4], 5: names_columns[5], 6: names_columns[6], 7: names_columns[7], 8: names_columns[8], 9: names_columns[9], 10: names_columns[10]}, inplace=True)
...
def watch_directory_app_brs(path):
...
watch_directory_app_brs(file_path)