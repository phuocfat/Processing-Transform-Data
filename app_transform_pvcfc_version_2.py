...
def function_formatter(file_path_raw_data, file_path_data_formated, name_file, len_catalog):
    ...
    search_string = ["4. Tồn kho","4.1 Ure rời","4.2 Ure bao", "- Loại bao thương mại", "- Loại bao không thương mại", "4.3. N46Plus bao", "4.4. Ure Bio bao", "4.5. N46 True bao", "4.6. N46 Neb bao", "4.7. N46 Đen bao"]
    from_to = []
    ...
    string = ['3. NH3 đưa về bồn', '4. NH3 xuất bán', '5. NH3 tồn kho']
    ...
    df = pd.concat([df.iloc[:index+4, :],df.iloc[index+4:, :]], ignore_index=True)
    ...
    index_start = module_process_data.get_row(df, 'F. TÌNH HÌNH HOẠT ĐỘNG', 1)
    ...
    index_remove = module_process_data.get_row(df,'Đóng bao',3)
    
    #
    df.iloc[index_remove:index_remove+8, 4] = ''
...
#function_formatter(file_path_raw_data_file_xlsx, file_path_data_formated, "ZPP_PG_BCNC_PVCFC", len_catalog)
